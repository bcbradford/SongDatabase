''' Dialog window that updates an entry in the database '''
...
class UpdateEntryDialog(QDialog):
    ...
    def _create_input_combo_box(self):
        self._input_label = QLabel("Update: ")
    ...
    def _submit_clicked(self):
        ...
        self._execute_update(table, item_string)
        self._update_input_cb()
        self._update_table_view()
        
    def _execute_update(self, table, item_string):
        query = ""
        dialog = None

        match (table):
            case 'Song':
                query = qs.update_song()
                dialog = init_input_dialog("Song", ["Title"])
            case 'Artist':
                query = qs.update_artist()
                dialog = init_input_dialog("Artist", ["Name"])
            case 'Category':
                query = qs.update_category()
                dialog = init_input_dialog("Category", ["Name"])
            case 'Album':
                query = qs.update_album_title()
                dialog = init_input_dialog("Album", ["Title"])
            case _:
                self._display_message("Error", f"Invalid table {table}")
                return

        dialog.exec()
        values = [*dialog.get_inputs(), item_string]
        if values[0] == "": return
...
def init_update_entry_dialog(db):
    return UpdateEntryDialog(db)
